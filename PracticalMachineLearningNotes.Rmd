...
testing <- mutate(vowel.test, y = as.factor(y))
```
Then set the seed to 33833. Fit (1) a random forest predictor relating the factor variable y to the remaining variables and (2) a boosted predictor using `method = "gbm"`. Fit these both with the `train()` command in the `caret` package.  
```{r}
set.seed(33833)
rfmod <- train(y~., data = training, method = "rf", prox = TRUE)
gbmmod <- train(y~., data = training, method = "gbm", verbose = FALSE)
```

What are the accuracies for the two approaches on the test data set? What is the accuracy amoung the test set samples where the two methods agree?
```{r}
rfacc <- confusionMatrix(testing$y, predict(rfmod, testing))
gbmacc <- confusionMatrix(testing$y, predict(gbmmod, testing))
agree <- predict(rfmod, testing) == predict(gbmmod, testing)
bothacc <- confusionMatrix(testing$y[agree], predict(gbmmod, testing[agree,]))

data.frame(RF_Accuracy = rfacc$overall[[1]],
           GMB_Accuracy = gbmacc$overall[[1]],
           Agreement_Accuracy = bothacc$overall[[1]])
```
...
```{r}
set.seed(62433)
rfmod <- train(diagnosis~., data = training, method = "rf", prox = TRUE)
gbmmod <- train(diagnosis~., data = training, method = "gbm", verbose = FALSE)
ldamod <- train(diagnosis~., data = training, method = "lda")
pred1 <- predict(rfmod, testing)
pred2 <- predict(gbmmod, testing)
pred3 <- predict(ldamod, testing)
...
```{r}
rfacc <-  confusionMatrix(pred1   ,testing$diagnosis)
gbmacc <- confusionMatrix(pred2   ,testing$diagnosis)
ldaacc <- confusionMatrix(pred3   ,testing$diagnosis)
comacc <- confusionMatrix(combPred,testing$diagnosis)
data.frame(RF_Accuracy = rfacc$overall[[1]],
           GMB_Accuracy = gbmacc$overall[[1]],
           LDA_Accuracy = ldaacc$overall[[1]],
           COMB_Accuracy = comacc$overall[[1]])
```

3. Load the concrete data  
```{r}
set.seed(3523)
library(AppliedPredictiveModeling)
data(concrete)
...
4. Load the data ont he number of visitors tot he instructors blog from `https://d396qusza40orc.cloudfront.net/predmachlearn/gaData.csv`  
```{r}
library(lubridate)
url <- "https://d396qusza40orc.cloudfront.net/predmachlearn/gaData.csv"
dest <- paste0(getwd(), "/Data/Quiz4Q4Data.csv")
...
library(forecast)
modFit <- bats(tstrain)
fcast <- forecast(modFit, level = 95, h = length(testing[,1]))
```
For how many of the testing points is the true value within the 95% prediction interval bounds?  
```{r}
sum(fcast$lower < testing$visitsTumblr & 
      testing$visitsTumblr < fcast$upper) / length(testing[,1])
```


5. Load the concrete data  
```{r}
set.seed(3523)
